...
class TvSeriesRepositoryImpl implements TvSeriesRepository {
  ...
  Future<Either<Failure, List<TvSeries>>> getOnTheAirTvSeries() async {
    try {
      ...
      return Right(result.map<TvSeries>((model) => model.toEntity()).toList());
    } on ServerException {
  ...
  Future<Either<Failure, List<TvSeries>>> getTvSeriesRecommendations(
      int id) async {
    try {
      final result = await remoteDataSource.getTvSereisRecommendations(id);
      return Right(result.map<TvSeries>((model) => model.toEntity()).toList());
    } on ServerException {
      return Left(ServerFailure(''));
    } on SocketException {
  ...
  Future<Either<Failure, String>> removeWatchlist(TvSeriesDetail movie) async {
    try {
      final result = await localDataSource
          .removeWatchlistTv(TvSeriesTable.fromEntity(movie));
      return Right(result);
  ...
  Future<Either<Failure, String>> saveWatchlist(TvSeriesDetail movie) async {
    try {
      final result = await localDataSource
          .insertWatchlistTv(TvSeriesTable.fromEntity(movie));
      return Right(result);
    } on DatabaseException catch (e) {
      return Left(DatabaseFailure(e.message));
    } catch (e) {
