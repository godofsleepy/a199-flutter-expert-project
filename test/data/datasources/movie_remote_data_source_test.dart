...
void main() {
  ...
  group('get Now Playing Series', () {
    final tData = TvSeriesResponse.fromJson(
            json.decode(readJson('dummy_data/ontheair_tvseries.json')))
        .results;
    ...
    test('should return list of series Model when the response code is 200',
      ...
      when(mockHttpClient.get(Uri.parse('$BASE_URL/tv/on_the_air?$API_KEY')))
          .thenAnswer((_) async => http.Response(
              readJson('dummy_data/ontheair_tvseries.json'), 200));
      // act
  ...
  group('get Popular Series', () {
    ...
    test('should return list of series when response is success (200)',
      ...
      when(mockHttpClient.get(Uri.parse('$BASE_URL/tv/popular?$API_KEY')))
          .thenAnswer((_) async => http.Response(
                  readJson('dummy_data/popular_tvseries.json'), 200, headers: {
  ...
  group('get series detail', () {
    final tId = 1;
    final tData = DetailTvSeriesModel.fromJson(
    ...
    test('should return series detail when the response code is 200', () async {
      ...
      when(mockHttpClient.get(Uri.parse('$BASE_URL/tv/$tId?$API_KEY')))
          .thenAnswer((_) async => http.Response(
                  readJson('dummy_data/series_detail.json'), 200, headers: {
                HttpHeaders.contentTypeHeader: 'application/json; charset=utf-8'
              }));
      // act
  ...
  group('get series recommendations', () {
    ...
    test('should return list of series Model when the response code is 200',
      ...
      // act
      final result = await dataSource.getTvSereisRecommendations(tId);
      // assert
      expect(result, equals(tData));
    });
    ...
    test('should throw Server Exception when the response code is 404 or other',
      ...
      when(mockHttpClient
          ...
          .thenAnswer((_) async => http.Response('Not Found', 404));
      // act
      final call = dataSource.getTvSereisRecommendations(tId);
      // assert
      expect(() => call, throwsA(isA<ServerException>()));
    });
  });
  ...
  group('search series', () {
    final tData = TvSeriesResponse.fromJson(
            json.decode(readJson('dummy_data/search_chucky_series.json')))
        .results;
    final tQuery = 'Spiderman';
    ...
    test('should return list of series when response code is 200', () async {
      ...
      when(mockHttpClient
          ...
          .thenAnswer((_) async => http.Response(
                  readJson('dummy_data/search_chucky_series.json'), 200,
                  headers: {
                    HttpHeaders.contentTypeHeader:
                        'application/json; charset=utf-8'
                  }));
